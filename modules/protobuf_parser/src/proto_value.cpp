...
{
    if (type == "int32")       return PB_INT32;
    else if (type == "uint32") return PB_UINT32;
    else if (type == "int64")  return PB_INT64;
    else if (type == "uint64") return PB_UINT64;
    else if (type == "float")  return PB_FLOAT;
    else if (type == "double") return PB_DOUBLE;
    else if (type == "bool")   return PB_BOOL;
    else if (type == "string") return PB_STRING;
    else
...
{
    if (type == PB_INT32 || type == PB_UINT32 || type == PB_FLOAT)
        return 4;
    else if (type == PB_INT64 || type == PB_UINT64 || type == PB_DOUBLE)
        return 8;
    else if (type == PB_BOOL)
        return 1;
    else
        CV_Error(Error::StsNotImplemented, format("Could not determine a size of type [%d]", type));
    return 0;
}
...
ProtoValue::ProtoValue(const std::string& t, bool _packed, const std::string& defaultValue)
    : ProtobufField(getProtoType(t))
{
    packed = _packed;
    defaultValueStr = defaultValue;
}
...
void ProtoValue::read(std::istream& s)
{
    ...
    {
        const int bytesPerValue = sizeOf(_type);
        if (packed)
        {
...
void ProtoValue::read(std::vector<std::string>::iterator& tokenIt)
{
    ...
    if (_type != PB_STRING)
    {
        data.resize(data.size() + sizeOf(_type));
...
Ptr<ProtobufField> ProtoValue::operator[](int idx) const
{
    ...
    if (!data.empty())
    {
        ...
        int numBytes = (idx != (int)offsets.size() - 1 ? offsets[idx + 1] : data.size()) - offsets[idx];
        const char* value = &data[offsets[idx]];
        p->data = std::vector<char>(value, value + numBytes);
        p->offsets = std::vector<int>(1, 0);
    }
    return p;
}
...
std::string ProtoValue::getString(int idx) const
{
    ...
    if (!data.empty())
    {
        if (idx < 0 || (size_t)idx >= offsets.size())
            CV_Error(Error::StsOutOfRange, format("Index [%d] out of range [0, %d)", idx, offsets.size()));
        int numBytes = ((size_t)idx != offsets.size() - 1 ? offsets[idx + 1] : data.size()) - offsets[idx];
        const char* ptr = &data[offsets[idx]];
        return std::string(ptr, ptr + numBytes);
    }
