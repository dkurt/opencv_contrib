...
void ProtobufNode::operator << (const std::string& str)
{
    CV_Assert(type() == PB_STRING, fields.size() == 1);

    std::vector<std::string> value(1, str);
    std::vector<std::string>::iterator valueIt = value.begin();
    fields[0]->clear();
    fields[0].dynamicCast<ProtoValue>()->read(valueIt);
}
